...
- Аккаунт хранится в `localStorage` под ключом `gamehub.session` (был `civa.session` — переименован
  2026-07-16 вместе с JWT-issuer'ом `civa`→`gamehub`; разово разлогинило всех, кто был залогинен).
